...
. Edit config.rs (env vars),  handlers.rs (mcp tools), and Cargo.toml (additionnal crates needed by your business logic) 
...
[source,console]
docker buildx build   --target mcp-searxng-bridge   -t davidburet/mcp-searxng-bridge:latest   --platform linux/amd64   --push .
